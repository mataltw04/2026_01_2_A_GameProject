...
public class MeshTrali : MonoBehaviour
{
    ...
    private SkinnedMeshRenderer[] skinnedRenderer;  //캐릭터의 3D모델을 랜더링 하는 컴포넌트들
    private bool isTrailActive;                     //현재 잔상 효과가 활성화되어 있는지를 확인하는 변수
    ...
    IEnumerator ActivateTrali(float timeActivated)
    {
        normalSpeed = moveScript.movementSpeed;                //현재 속도를 저장하고 증가돤 속도 적용
    ...
    void Update()
    {
        if(Input.GetKeyDown(KeyCode.Space) && !isTrailActive)  //스페이스바를 누르고 현재 잔상 효과가 비 활성화 일 때
        {
            ...
            StartCoroutine(ActivateTrali(activeTime));         //잔상 효과 코루틴 시작 
        }
    }
}
